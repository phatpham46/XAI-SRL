...
class TasksParam:
    ...
    def __init__(self, taskFilePath):
        ...
        for i, (taskName, taskVals) in enumerate(self.taskDetails.items()):
            ...
            if "label_map_or_file" in taskVals:
                '''
                Label Map is the list of label names (or tag names in SRL) which are
                ...
                SRL. For classification tasks, if the labels are already numeric in data,
                ...
                if taskTypeMap[taskName] == TaskType.SRL:
    ...
    def validity_checks(self):
        ...
        for taskName, taskVals in self.taskDetails.items():
            ...
            # we definitely require label mapping for SRL task
            if taskVals["task_type"] == 'SRL':
                assert "label_map_or_file" in taskVals, "Unique Tags/Labels or map file needs to be mentioned in label_map_or_file for SRL"
