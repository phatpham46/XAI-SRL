...
def make_arguments(parser):
    ...
    parser.add_argument('--batch_size', type = int, default=32,
                        help='batch size to use for training, evaluating')
    ...
    parser.add_argument('--log_file', default='cal_brier_score.log', type = str,
...
logger = make_logger(name = "evaluate_brier_score", debugMode=args.debug_mode,
...
def main():
    ...
    allParams['num_train_steps'] = math.ceil(len(multiTaskDataLoaderTrain)/args.batch_size) *args.epochs // args.grad_accumulation_steps
    ...
    with torch.no_grad():
        evaluate(allDataDev, BatchSamplerDev, multiTaskDataLoaderDev, taskParams,
                model, gpu=allParams['gpu'],evalBatchSize=args.batch_size, hasTrueLabels=True, needMetrics=True)

        
    logger.info("\nRunning Evaluation on test...")
    with torch.no_grad():
        evaluate(allDataTest, BatchSamplerTest, multiTaskDataLoaderTest, taskParams,
                model, gpu=allParams['gpu'], evalBatchSize = args.batch_size, needMetrics=True, hasTrueLabels=True)                       
